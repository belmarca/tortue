...
main :: IO ()
main = do
  symTblHello <- initialSymbolTable hwSymbolTableStr hwEmptySymbolsCount
  symTblMinRepl <- initialSymbolTable minReplSymbolTableStr minReplEmptySymbolsCount
  defaultMain
    [ bgroup "RVM"
      [ bgroup "decoding symbol table"
        [ bench "HELLO!"   . whnfIO $ initialSymbolTable hwSymbolTableStr hwEmptySymbolsCount
        , bench "Min REPL" . whnfIO $ initialSymbolTable minReplSymbolTableStr minReplEmptySymbolsCount
        ]
      , bgroup "decoding instructions"
        [ bench "HELLO!"   . whnfIO $ benchDecodeInstr hwInstructionsStr symTblHello
        , bench "Min REPL" . whnfIO $ benchDecodeInstr minReplInstructionsStr symTblMinRepl
        ]
      ]
    ]
  where
...
minReplEmptySymbolsCount :: Int
minReplSymbolTableStr, minReplInstructionsStr :: String
(minReplSymbolTableStr, minReplEmptySymbolsCount, minReplInstructionsStr) = splitBytecode minReplStr
